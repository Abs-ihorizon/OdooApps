...
class AccountJournalInherit(models.Model):
    ...
    bank_balance = fields.Float(string="Bank Balance", compute='computeBankBalance')
    bank_statements_source = fields.Selection(selection=[
        ('manual', 'Record Manually'),
        ('undefined', 'Undefined')
    ], string='Bank Feeds', default='undefined', help="Defines how the bank statements will be registered")

    def computeBankBalance(self):
        search = self.env['account.move.line'].search([('account_id', '=', self.default_account_id.id)])
        amount = 0
        amount += sum([line.balance for line in search])
        self['bank_balance'] = amount

    def create_bank_statement(self):
        context = self._context.copy()
        action = self.env.ref('affinity_bank_reconciliation.action_bank_statement_wiz').read()[0]
        # action.update({
